...
void greyscale(float * dst, unsigned char * src, int w, int h)
{
	...
	{
		data[i] = src[i*4] + src[i * 4 + 1] + src[i * 4 + 2] / (3 * 255);
	}
}

void recolor(unsigned char * src, float * dst, int w, int h)
{
	for (int i = 0; i < screen_width*screen_height; i++)
	{
		src[i * 4] * 255;
